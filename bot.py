        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    # Pull out the formatted accounts text cleanly from the json data
                    formatted_data = res_json.get("data", {}).get("formattedAccounts", response.text)
                except Exception:
                    formatted_data = response.text

                # Send cleanly in a single code block so it's easy to copy
                await interaction.followup.send(
                    f"✅ **Purchase Successful ({self.category_name} x{amount})!**\n```ansi\n{formatted_data}\n```", 
                    ephemeral=True
                )
            else:
