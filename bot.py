...
class QuantitySelectView(View):
    ...
    async def select_quantity(self, interaction: discord.Interaction, select: Select):
        ...
        try:
            # 10-second timeout so it never hangs forever if the API lags
            response = requests.post(f"{API_BASE}/buy", json=payload, headers=HEADERS, timeout=10)
        except requests.exceptions.Timeout:
            await interaction.followup.send("❌ The purchase request timed out. The API is lagging.", ephemeral=True)
            return

        if response.status_code == 200:
            data = response.text
            # Sends tokens privately right here in the server channel so only you can see them
            await interaction.followup.send(f"✅ **Purchase Successful ({self.category_name} x{amount})!**\nHere are your session tokens:\n```json\n{data}```", ephemeral=True)
        else:
...
class CategorySelect(Select):
    def __init__(self, categories):
        ...
        for cat in categories:
            ...
            count = cat.get("count")
            price = cat.get("price")
            # Only list categories that actually have stock
            if count > 0:
...
class MogControlView(View):
    def __init__(self):
        super().__init__(timeout=None) # Persistent view that stays active 24/7
    ...
    async def balance_button(self, interaction: discord.Interaction, button: Button):
        try:
            response = requests.get(f"{API_BASE}/balance", headers=HEADERS, timeout=10)
        except requests.exceptions.Timeout:
...
class MogBot(discord.Client):
    ...
    async def setup_hook(self):
        # Register the persistent view so buttons work across restarts without expiring
