...
class QuantitySelectView(View):
    ...
    async def select_quantity(self, interaction: discord.Interaction, select: Select):
        ...
        if response.status_code == 200:
            try:
                # Send details securely to user's DMs
                await interaction.user.send(f"**🛒 Purchase Successful ({self.category_name} x{amount})!**\n```json\n{response.text}\n```")
                await interaction.followup.send("✅ Purchase successful! Check your Direct Messages for the session tokens.", ephemeral=True)
            except discord.Forbidden:
                await interaction.followup.send(f"❌ Purchased successfully, but I couldn't DM you! Here are your details:\n```json\n{response.text}```", ephemeral=True)
        else:
